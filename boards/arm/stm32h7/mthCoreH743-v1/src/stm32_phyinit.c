...
static void stm32_emac0_phy_enable(bool enable)
{
  ...
  if (enable && g_ethmac_handler != NULL)
    {
      ...
      stm32_gpiosetevent(GPIO_ETH_NINT, false, true, true, g_ethmac_handler, g_ethmac_arg);
  ...
  // up_mdelay(1);
  ...
  else
    {
      ...
      stm32_gpiosetevent(GPIO_ETH_NINT, false, false, false,NULL, NULL);
    }
}
#endif
...
#ifdef CONFIG_NETINIT_MONITOR
int arch_phy_irq(const char *intf, xcpt_t handler, void *arg, phy_enable_t *enable)
{
